...
export const register = (userName, email, userPassword) => async (dispatch) => {
  try {
    dispatch({
...
export const getUserDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:8080/api/v1/users/${id}`, config)
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(`http://localhost:8080/api/v1/users/profile/${user.id}`, user, config)
...
export const listUsers = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:8080/api/v1/users`, config)
